...
export class SeasonalBackgroundsModel extends DatabaseModel {
  ...
  private static _instance: SeasonalBackgroundsModel;

  private constructor() {
    super();

    if (!SeasonalBackgroundsModel._instance) {
      SeasonalBackgroundsModel._instance = this;
    }

    this.minExpiry = new Date();

    return SeasonalBackgroundsModel._instance;
  }

  public static getInstance(): SeasonalBackgroundsModel {
    if (!SeasonalBackgroundsModel._instance) {
      SeasonalBackgroundsModel._instance = new SeasonalBackgroundsModel();
    }
    return SeasonalBackgroundsModel._instance;
  }
  ...
  async cacheBackgrounds(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const prismaClient = this._databaseService.getClient();
