      ...
      .then(async (verify) => {
        if (
          ...
          const score_id = await conn.query(
            ...
            [Number(req.body.ruleset_id), dbResToken[0].user_id]
          );
          ...
          */
          ...
          res.send({
            accuracy: Number(req.body.accuracy),
            beatmap_id: Number(url[3]),
            best_id: null, // TODO: Return best Score ID
            build_id: 6394, // TODO: Build ID, Build Validation
            current_user_attributes: {
              pin: null, // TODO: Score Pin
            },
            ended_at: new Date(Date.now()).toISOString(),
            id: Number(score_id[0].score_id),
            legacy_perfect: null, // TODO: Legacy Perfect?
            max_combo: Number(req.body.max_combo),
            mods: req.body.mods,
            passed: Boolean(req.body.passed),
